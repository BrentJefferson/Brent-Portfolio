            ...
            <div className="flex flex-row flex-wrap gap-3 lg:gap-4 justify-center">
              {group.items.map((item) => {
                ...
                return (
                  <div
                    key={item.name}
                    className="flex items-center gap-2 px-3 py-2 lg:px-5 lg:py-3 bg-white/10 rounded-xl toolkit-pill"
                    style={{ animationDelay: delay }}
                  >
                    <img src={item.src} alt={item.name} className="w-5 h-5 lg:w-7 lg:h-7 object-contain" />
